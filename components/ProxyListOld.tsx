...
const ProxyList = React.memo(function ProxyList({ 
  ...
  const filteredProxies = useMemo(() => {
    ...
    const matchesStatus = filterStatus === 'all' || proxy.status === filterStatus
      ...
      return matchesSearch && matchesType && matchesStatus && matchesAnonymity && matchesCountry && matchesGroup
    })
  }, [proxies, searchTerm, filterType, filterStatus, filterAnonymity, filterCountry, filterGroup])
  ...
  const uniqueGroups = useMemo(() => 
    Array.from(new Set(proxies.map(p => p.group).filter(Boolean))).sort(), 
    [proxies]
  )
  ...
  const getAnonymityColor = useCallback((anonymity?: string) => {
    switch (anonymity) {
  ...
  const getAnonymityText = useCallback((anonymity?: string) => {
    switch (anonymity) {
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                ...
                <select
                  value={filterCountry}
                  onChange={(e: ChangeEvent<HTMLSelectElement>) => setFilterCountry(e.target.value)}
              ...
              <div>
                ...
                <select
                  value={filterGroup}
                  onChange={(e: ChangeEvent<HTMLSelectElement>) => setFilterGroup(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
              ...
              <div>
                ...
                <select
                  value={filterPing}
                  onChange={(e: ChangeEvent<HTMLSelectElement>) => setFilterPing(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
              ...
              <div>
                ...
                <select
                  value={filterSpeed}
                  onChange={(e: ChangeEvent<HTMLSelectElement>) => setFilterSpeed(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
                    ...
                    {editingId === proxy.id ? (
                      <div className="space-y-2">
                        <input
                          type="text"
                          value={editForm.host || ''}
                          onChange={(e: ChangeEvent<HTMLInputElement>) => setEditForm({...editForm, host: e.target.value})}
                        ...
                        <input
                          type="number"
                          value={editForm.port || ''}
                          onChange={(e: ChangeEvent<HTMLInputElement>) => setEditForm({...editForm, port: parseInt(e.target.value)})}
                          className="input text-sm"
                          placeholder="Port"
                        />
                      </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {editingId === proxy.id ? (
                      <select
                        ...
                        onChange={(e: ChangeEvent<HTMLSelectElement>) => setEditForm({...editForm, type: e.target.value as any})}
                        className="input text-sm"
                      >
                        <option value="http">HTTP</option>
                        <option value="https">HTTPS</option>
